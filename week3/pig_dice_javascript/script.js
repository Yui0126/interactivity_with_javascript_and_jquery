(function() {
    ...
    function throwDice() {
        ...
        if (gameData.rollSum === 2) {
            ...
            gameData.index ? (gameData.index = 0) : (gameData.index = 1); //true or false statement. if gameData.index = 1, it is true. if 0 it is false. <TRUE:FALSE>
            showCurrentScore();
            setTimeout(setUpTurn, 2000);
        } else if (gameData.roll1 === 1 || gameData.roll2 === 1) {
            gameData.index ? (gameData.index = 0) : (gameData.index = 1);
            game.innerHTML += `<p>a 1 was rolled. Sorry, your turn is over, switching to ${gameData.players[gameData.index]}</p>`;
            setTimeout(setUpTurn, 2000);
        } else {
